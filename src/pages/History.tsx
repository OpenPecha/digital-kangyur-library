...
const GanttTooltip = ({
  ...
  return <div className="bg-white p-3 rounded-md shadow-md border border-primary-3/20">
      <p className="font-semibold text-primary-1">
        {data.name}
        {data.tibetanName && <span className="tibetan ml-2">{data.tibetanName}</span>}
      </p>
...
const EventMarker = ({
  ...
  const flatEvents = periodData.flatMap(period => period.events.map(event => ({
    ...event,
  ...
  return <div className="relative h-0">
      ...
      return <div key={index} className="absolute transform -translate-x-1/2 cursor-pointer animate-fade-in" style={{
        left: `${xPos}px`,
        top: '-12px'
        ...
        <div className="container mx-auto px-4">
            ...
            <CardContent className="p-6">
              <div ref={chartRef} className="w-full">
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={chartData} layout="vertical" barCategoryGap={20} margin={{
                  top: 40,
                  right: 30,
                  left: 150,
                  bottom: 30
                    ...
                    <XAxis type="number" domain={[minYear, maxYear]} tickCount={10} ticks={yearMarkers} />
                    <YAxis type="category" dataKey="name" width={140} tickLine={false} axisLine={false} />
                    <Tooltip content={<GanttTooltip />} />
                    {yearMarkers.map(year => <ReferenceLine key={year} x={year} stroke="#e0e0e0" strokeDasharray="3 3" />)}
                    <Bar dataKey="duration" background={{
                    fill: '#f5f5f5'
                  }} stackId="a" barSize={25} shape={props => {
                    const {
                      x,
                      y,
                      width,
                    ...
                    const radius = 4;
                    return <g>
                            <rect x={x} y={y} width={width} height={height} fill={payload.color} rx={radius} ry={radius} />
                          </g>;
                  }} />
                  </BarChart>
                </ResponsiveContainer>
                
                <EventMarker periodData={timelineData} minYear={minYear} maxYear={maxYear} chartWidth={chartWidth - 180} // Adjust for chart margins
          ...
          {selectedEvent && <Card className="mb-8 animate-fade-in border-secondary-1 shadow-md">
              <CardContent className="p-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button onClick={() => setSelectedEvent(null)} className="text-gray-500 hover:text-primary-1">
                    ×
                  </button>
                </div>
          ...
          <ScrollArea className="w-full mb-12">
            ...
            paddingRight: '1.5rem'
              ...
              {timelineData.map(period => <Card key={period.id} className="hover:shadow-md transition-shadow" style={{
              minWidth: '350px',
              maxWidth: '400px'
            }}>
                  <CardContent className="p-4">
                    <h3 className="text-xl font-bold text-primary-1 mb-2">
                      ...
                      {period.tibetanPeriod && <span className="language-tibetan tibetan ml-2">{period.tibetanPeriod}</span>}
                    </h3>
                    <p className="text-sm text-gray-600 mb-3">{period.startYear} - {period.endYear}</p>
                    <ul className="pl-5 list-disc space-y-2">
                      {period.events.map((event, idx) => <li key={idx} className="text-gray-700">
