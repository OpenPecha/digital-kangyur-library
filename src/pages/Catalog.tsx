...
const Catalog = () => {
  ...
  const [expandedItems, setExpandedItems] = useState<string[]>(['vinaya']); // Start with Vinaya expanded
  ...
  const getSelectedItemDetails = () => {
    // First look in main categories
    const mainItem = catalogData.find(item => item.id === selectedItem);
    if (mainItem) return mainItem;
    
    // Then look in subcategories
    for (const category of catalogData) {
      if (category.children) {
        const subItem = category.children.find(item => item.id === selectedItem);
  ...
  const filteredCatalog = searchQuery
    ? catalogData.map(category => ({
        ...
        children: category.children?.filter(item => 
          ...
          item.title.tibetan.includes(searchQuery)
        )
      })).filter(category => 
        category.title.english.toLowerCase().includes(searchQuery.toLowerCase()) ||
        category.title.tibetan.includes(searchQuery) ||
      ...
      <div className="bg-gradient-to-r from-kangyur-maroon to-kangyur-orange text-white pt-32 pb-16 relative overflow-hidden">
        ...
        <div className="container relative mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <h1 className="text-4xl sm:text-5xl font-bold mb-4">Kangyur Catalog</h1>
            <p className="tibetan text-2xl mb-4">བཀའ་འགྱུར་དཀར་ཆག།</p>
            <p className="text-xl text-white/80 mb-8">
              Browse the comprehensive catalog of the Kangyur collection, organized according to traditional divisions.
            </p>
            ...
            <div className="bg-white border border-kangyur-orange/10 rounded-xl shadow-sm p-5">
              ...
              <div className="space-y-2">
                {filteredCatalog.map((category) => (
                  <div key={category.id} className="border-b border-kangyur-orange/10 last:border-0 pb-2 last:pb-0">
                    <div
                      className={cn(
                        ...
                        {category.children.map((child) => (
                          <div
                            ...
                            className={cn(
                              ...
                              selectedItem === child.id ? "bg-kangyur-orange/10 text-kangyur-orange" : "hover:bg-kangyur-orange/5"
                            )}
                            onClick={() => handleSelectItem(child.id)}
                          >
                            <Book className="w-4 h-4 text-kangyur-orange/70 mr-3" />
                            <div>
                              <div className="font-medium text-sm">
                                {child.title.english}
                              </div>
                              <div className="tibetan text-xs text-kangyur-dark/70">
                                {child.title.tibetan}
                              </div>
                            </div>
                          </div>
