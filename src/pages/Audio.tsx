        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="max-w-md mx-auto">
            <div className="relative">
              ...
              <input type="search" className="block w-full pl-10 pr-4 py-3 bg-white/10 border border-white/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-white/30 text-white placeholder-white/60 font-sans" placeholder="Search audio recordings..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            ...
            {selectedAudio && <span className="text-sm text-gray-500">
                Now playing: {selectedAudio.title.english}
              </span>}
          </div>
          <Button variant="outline" size="sm" onClick={() => setShowCatalog(!showCatalog)} className="text-kangyur-maroon hover:text-kangyur-orange transition-colors border-kangyur-maroon/20 hover:border-kangyur-orange">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className={cn("transition-all duration-300", showCatalog ? "lg:col-span-3" : "lg:col-span-4")}>
            {selectedAudio ? <AudioPlayer audio={selectedAudio} /> : <div className="bg-white rounded-xl shadow-md p-12 text-center border border-kangyur-orange/10">
                <div className="max-w-md mx-auto">
                  ...
                  <div className="space-y-3">
                    <Button onClick={() => handleSelectAudio('heart-sutra')} className="w-full bg-kangyur-orange hover:bg-kangyur-orange/90 text-white py-3">
                      🎵 Listen to Heart Sutra
                    </Button>
                    <Button onClick={() => handleSelectAudio('diamond-sutra')} variant="outline" className="w-full border-kangyur-maroon text-kangyur-maroon hover:bg-kangyur-maroon hover:text-white py-3">
                      🔹 Try Diamond Sutra
                    </Button>
                  </div>
                </div>
        ...
        {!selectedAudio && <div className="mt-12">
            ...
            duration: '42:18',
            category: 'Sutras'
          }].map(item => <div key={item.id} onClick={() => handleSelectAudio(item.id)} className="bg-white p-4 rounded-lg shadow-sm border border-kangyur-orange/10 hover:shadow-md transition-shadow cursor-pointer group">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs text-kangyur-orange font-medium">{item.category}</span>
                    <span className="text-xs text-gray-500">{item.duration}</span>
                  </div>
                  <h4 className="font-medium text-kangyur-maroon group-hover:text-kangyur-orange transition-colors">
                    {item.title}
                  </h4>
                </div>)}
            </div>
