...
const TextCard = ({
  ...
  return <Link to={`/texts/${id}`} className={cn("group block overflow-hidden transition-all duration-300", isCompact ? "h-full" : "", isFeatured ? "bg-gradient-to-br from-kangyur-cream to-white border border-kangyur-orange/20 rounded-xl shadow-sm hover:shadow-md" : "bg-white border border-kangyur-orange/10 rounded-xl shadow-sm hover:shadow-md", className)}>
      <div className={cn("flex", isCompact ? "flex-col h-full" : "flex-col")}>
        {/* Image - only shown for compact variant */}
        {imageUrl && isCompact && <div className="h-36 overflow-hidden">
        ...
        {/* Content */}
        <div className={cn("flex flex-col", isCompact ? "p-4 flex-grow" : isFeatured ? "p-6" : "p-5")}>
          ...
          {/* Titles */}
          <div className="mb-3">
            <h3 className={cn("tibetan font-medium text-kangyur-orange", isCompact ? "text-base" : isFeatured ? "text-xl" : "text-lg")}>
              {title.tibetan}
            </h3>
            
            <h2 className={cn("font-semibold text-kangyur-dark group-hover:text-kangyur-green transition-colors", isCompact ? "text-base" : isFeatured ? "text-xl" : "text-lg")}>
              {title.english}
            </h2>
            
            {title.sanskrit && !isCompact && <p className="text-sm text-kangyur-dark/70 italic mt-1">
